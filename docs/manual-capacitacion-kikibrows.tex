...
Al iniciar sesi\'on en \textbf{login}, el sistema deriva autom\'aticamente
seg\'un el rol de la cuenta:
\begin{itemize}[leftmargin=1.6em, itemsep=2pt, topsep=2pt]
  \item \textbf{Alumna} $\rightarrow$ vuelve a la p\'agina de Inicio, desde donde
        usa el men\'u superior para entrar a \emph{Mis Cursos} y dem\'as.
  \item \textbf{Administradora} $\rightarrow$ es llevada directamente al
        \emph{Panel de Administraci\'on}. Tambi\'en puede entrar desde el
        men\'u de usuaria (opci\'on <<Panel Admin>>) si ya tiene sesi\'on abierta.
\end{itemize}
